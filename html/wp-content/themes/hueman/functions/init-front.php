    ...
    ?>
      <?php do_action( '__before_content_section', $tmpl ); ?>
        <section class="content">
          ...
          <div class="pad group" style="position: relative; top: 60px; height: 600px;"> <!-- Changed -->
            <?php
              foreach ( $seks as $_id ) {
                  if ( 'wp' == $_id )
    ...
    function hu_print_mobile_btn() {
      ...
      <?php else : ?>
        ...
        <div class="ham__navbar-toggler-two collapsed" title="Menu" aria-expanded="false">
          ...
          <i class="fa fa-search" aria-hidden="true"></i> <!-- Changed -->
        </div>
      <?php endif; ?>
      <?php
    }
...
if ( ! function_exists( 'hu_print_title' ) ) {
    function hu_print_title( $echo = true ) {
        // Text or image?
        // Since v3.2.4, uses the WP 'custom_logo' theme mod option. Set with a filter.
        $is_image = false;
        
        $logo_or_title = '<img src="https://www.thunderthought.com/wp-content/uploads/2017/05/Picture1-3.png" class = "blog-title">'; // Changed //

        ob_start();
          ?>
            <p class="site-title-title"><?php hu_do_render_logo_site_tite( $logo_or_title ) ?></p>
          <?php
        $html = ob_get_contents();
        if ($html) ob_end_clean();
        if ( $echo )
...
if ( ! function_exists( 'hu_print_title_mobile' ) ) {
    function hu_print_title_mobile( $echo = true ) {
        // Text or image?
        // Since v3.2.4, uses the WP 'custom_logo' theme mod option. Set with a filter.
        $is_image = false;
        
        $logo_or_title = '<img src="https://www.thunderthought.com/wp-content/uploads/2017/05/Picture1-3.png" class = "blog-title-mobile">';

        ob_start();
          ?>
            <p class="site-title-mobile"><?php echo $logo_or_title ?><font size = "2%" color="black" style="position: absolute; left: 35%; font-style: italic; font-family: cursive; color: rgb(60, 60, 60); font-size: 100%">- The Thrill of Thoughts</font></p> <!-- Changed -->
          <?php
        $html = ob_get_contents();
        if ($html) ob_end_clean();
        if ( $echo )
...
if ( ! function_exists( 'hu_blog_title' ) ) {
  ...
  function hu_blog_title() {
    ...
    //     $subheading
    //   ) Changed
    return apply_filters( 'hu_blog_title', sprintf('%1$s',
        $heading
      )
    );
  }
